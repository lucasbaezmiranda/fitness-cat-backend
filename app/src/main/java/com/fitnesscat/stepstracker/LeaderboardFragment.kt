...
class LeaderboardFragment : Fragment() {
    ...
    private fun updateLeaderboard() {
        ...
        val userSteps = userPreferences.getTodayStepCount().toFloat()
        ...
        val users = listOf(
            LeaderboardUser("You", userSteps.toInt(), true),
        ...
        users.forEachIndexed { index, user ->
            ...
            when (user.name) {
                "You" -> {
                    userStepsText.text = "${user.steps} steps"
                    userRankText.text = "#$rank"
                }
                "Amigo1" -> {
                    amigo1StepsText.text = "${user.steps} steps"
                    amigo1RankText.text = "#$rank"
                }
                "Amigo2" -> {
                    amigo2StepsText.text = "${user.steps} steps"
